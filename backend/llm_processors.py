...
import re
import ast
from beatoven_module import extract_prompt_and_lyrics
...
class ImageToLyricsProcessor(BaseLLMProcessor):
    ...
    def _build_messages(self):
        if self.language == "en":
            if self.chords:
                prog = " - ".join(self.chords)
                chord_text = (
                    f"The user's audio has the chord progression: {prog}. "
            ...
            if self.chords:
                prog = " - ".join(self.chords)
                chord_text = f"用户上传的音频和弦进行为: {prog}。请结合这些信息。\n\n"
            else:
                chord_text = ""
...
class ImageToVisualEntitiesProcessor(BaseLLMProcessor):
    ...
    def _postprocess(self, output: str):
        ...
        try:
            ...
            return [
                f"{str(x).strip()} {random.choice(art_forms)}"
            ...
            return [
                f"{tag.strip()} {random.choice(art_forms)}"
                for tag in tags
            ]