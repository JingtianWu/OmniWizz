...
## Features
- **Image → Music** via Beatoven.ai and GPT-4.1-mini lyrics
...
- Toggle `TEST_MODE` (via environment variable or in `backend/config.py`) for offline demos.
  ...
  GPT-4.1-mini and Beatoven.ai services, suitable for memory-constrained deployments.
...
│   ├── beatoven_module.py   ← generate music with Beatoven.ai
...
```
  ...
  `.env` file if present. Set `TEST_MODE=false` in the environment to enable real API calls. When disabled,
   the backend submits prompts to the Beatoven.ai service via
   `POST https://public-api.beatoven.ai/api/v1/tracks/compose` and polls `GET /api/v1/tasks/{task_id}`
   until completion.
...
For offline demos or memory-constrained deployments, enable `TEST_MODE` either in `backend/config.py` or via an environment variable. This skips calling the remote GPT-4.1-mini and Beatoven.ai services so the API and frontend can run without external dependencies.
