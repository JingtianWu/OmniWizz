...
- **Image → Related Images** via OpenAI Image Generation (`OPENAI_API_KEY` required)
...
│   ├── openai_image_module.py ← related image generation via OpenAI
...
```
  ...
  Set the `OPENAI_API_KEY`, `PIAPI_KEY`, and `MUSIC_AI_API_KEY` environment variables
